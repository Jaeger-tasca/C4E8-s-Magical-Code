package dev.c4e8.mod.gui.clickgui.components.impl;

import dev.c4e8.C4E8;
import dev.c4e8.api.utils.math.Animation;
import dev.c4e8.mod.modules.impl.client.ClickGui;
...
import dev.c4e8.mod.gui.clickgui.components.Component;
import dev.c4e8.mod.gui.clickgui.ClickGuiScreen;
import dev.c4e8.mod.gui.clickgui.tabs.ClickGuiTab;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : C4E8.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : C4E8.GUI.getColor());
		}
