...
import static dev.c4e8.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (C4E8.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
        ...
        if (this.rotate.getValue() == RotateMode.Bypass) {
            C4E8.ROTATION.snapAt(C4E8.ROTATION.rotationYaw, 90);
        }
    ...
    private void gotoPos(BlockPos offPos) {
        ...
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, C4E8.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && C4E8.BREAK.isMining(pos)) {
            return false;
        }
