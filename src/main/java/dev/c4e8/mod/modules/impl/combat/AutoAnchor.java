...
import dev.c4e8.api.utils.math.AnimateUtil;
...
import static dev.c4e8.api.utils.world.BlockUtil.*;
...
public class AutoAnchor extends Module {
	...
	public AutoAnchor() {
		...
		setChinese("自动重生锚");
		INSTANCE = this;
		C4E8.EVENT_BUS.subscribe(new AnchorRender());
	}
	...
	public void onUpdate() {
		...
		if (currentPos != null) {
			...
			boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || C4E8.BREAK.isMining(currentPos, false));
			if (shouldSpam) {
	...
	public BlockPos getHelper(BlockPos pos) {
		for (Direction i : Direction.values()) {
			if (checkMine.getValue() && C4E8.BREAK.isMining(pos.offset(i))) continue;
