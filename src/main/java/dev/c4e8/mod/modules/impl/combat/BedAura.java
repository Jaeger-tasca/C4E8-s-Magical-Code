...
import dev.c4e8.api.events.impl.RotateEvent;
import dev.c4e8.api.events.impl.UpdateWalkingPlayerEvent;
import dev.c4e8.api.utils.combat.CombatUtil;
import dev.c4e8.api.utils.entity.EntityUtil;
import dev.c4e8.api.utils.entity.InventoryUtil;
import dev.c4e8.api.utils.math.ExplosionUtil;
import dev.c4e8.api.utils.math.FadeUtils;
import dev.c4e8.api.utils.math.Timer;
...
public class BedAura extends Module {
	...
	public void onEnable() {
		lastYaw = C4E8.ROTATION.lastYaw;
		lastPitch = C4E8.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(C4E8.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !C4E8.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !C4E8.BREAK.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(C4E8.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(C4E8.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			C4E8.ROTATION.lookAt(directionVec);
			return true;
		} else {
			this.directionVec = directionVec;
			if (C4E8.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
