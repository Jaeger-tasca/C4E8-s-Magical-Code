...
public class HoleESP extends Module {
	...
	Type isHole(BlockPos pos) {
		if (mc.world.isAir(pos) && (!airYCheck.getValue() || pos.getY() == mc.player.getBlockY() - 1) && C4E8.HOLE.isHard(pos.up())) return Type.Air;
		int blockProgress = 0;
		boolean bedRock = true;
		for (Direction i : Direction.values()) {
			if (i == Direction.UP || i == Direction.DOWN) continue;
			if (C4E8.HOLE.isHard(pos.offset(i))) {
		...
		if (C4E8.HOLE.isDoubleHole(pos)) return Type.Normal;
		return Type.None;
	}
