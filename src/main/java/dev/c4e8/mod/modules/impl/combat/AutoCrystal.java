...
import dev.c4e8.api.utils.math.*;
import dev.c4e8.api.utils.render.ColorUtil;
import dev.c4e8.api.utils.render.JelloUtil;
...
import dev.c4e8.asm.accessors.IEntity;
...
import dev.c4e8.mod.modules.impl.exploit.Blink;
import dev.c4e8.mod.modules.impl.player.PacketMine;
import dev.c4e8.mod.modules.impl.render.ExplosionSpawn;
...
import static dev.c4e8.api.utils.world.BlockUtil.getBlock;
import static dev.c4e8.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        C4E8.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || C4E8.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
    ...
    private boolean faceVector(Vec3d directionVec) {
        if (!yawStep.getValue()) {
            ...
            if (C4E8.ROTATION.inFov(directionVec, fov.getValueFloat())) {
                return true;
            }
        }
