...
import dev.c4e8.mod.modules.impl.player.Freecam;
    ...
    public void onRender2D(DrawContext context, float tickDelta) {
        for (PlayerEntity ent : mc.world.getPlayers()) {
            ...
            if (vector.z > 0 && vector.z < 1) {
                ...
                if (pops.getValue() && C4E8.POP.getPop(ent.getName().getString()) != 0) {
                    final_string += " §bPop" + " " + Formatting.LIGHT_PURPLE + C4E8.POP.getPop(ent.getName().getString());
                }
                ...
                if (font.getValue() == Font.Fancy) {
                    FontRenderers.ui.drawString(context.getMatrices(), final_string, tagX, (float) posY - 10, C4E8.FRIEND.isFriend(ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB());
                } else {
                    context.getMatrices().push();
                    context.getMatrices().translate(tagX, ((float) posY - 11), 0);
                    context.drawText(mc.textRenderer, final_string, 0, 0, C4E8.FRIEND.isFriend(ent) ? friendColor.getValue().getRGB() : this.color.getValue().getRGB(), true);
                    context.getMatrices().pop();
                }
                context.getMatrices().pop();
            }
