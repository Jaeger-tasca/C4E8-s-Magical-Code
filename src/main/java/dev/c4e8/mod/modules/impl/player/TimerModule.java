...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !C4E8.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = C4E8.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					C4E8.TIMER.set((float) Math.max(C4E8.TIMER.getDefault(), timer));
				} else {
					C4E8.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
