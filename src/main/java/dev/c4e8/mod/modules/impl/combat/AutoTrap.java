...
import dev.c4e8.mod.modules.settings.Placement;
...
public class AutoTrap
    ...
    private void trapTarget(PlayerEntity target) {
        if (onlyHole.getValue() && !C4E8.HOLE.isHole(EntityUtil.getEntityPos(target))) return;
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (antiStep.getValue() && (C4E8.BREAK.isMining(pos.up(2)) || !onlyBreak.getValue())) {
        ...
        if (chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (!onlyBreaking.getValue() || C4E8.BREAK.isMining(pos.up(2))) {
                    tryPlaceObsidian(offsetPos);
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand, boolean web) {
        ...
        if (C4E8.BREAK.isMining(pos)) return;
