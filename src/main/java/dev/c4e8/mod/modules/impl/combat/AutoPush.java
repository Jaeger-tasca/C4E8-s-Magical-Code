...
import dev.c4e8.api.utils.math.MathUtil;
...
public class AutoPush extends Module {
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            C4E8.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            C4E8.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            C4E8.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            C4E8.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
            ...
            if (BlockUtil.clientCanPlace(piston)) {
                ...
                if (BlockUtil.getPlaceSide(piston, placeRange.getValue()) != null) {
                    ...
                    if (canPower) {
                        ...
                        if (side != null) {
                            if (rotate.getValue()) C4E8.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                            ...
                            if (rotate.getValue() && yawDeceive.getValue()) C4E8.ROTATION.lookAt(piston.offset(side), side.getOpposite());
...
/*                        if (mine.getValue()) {
    ...
    private boolean isTrueFacing(BlockPos pos, Direction facing) {
        ...
        float[] rotation = C4E8.ROTATION.getRotation(directionVec);
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!allowWeb.getValue() && C4E8.PLAYER.isInWeb(player)) return false;
        float[] offset = new float[]{-0.25f, 0f, 0.25f};
        ...
        return progress > surroundCheck.getValue() - 1 || C4E8.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
