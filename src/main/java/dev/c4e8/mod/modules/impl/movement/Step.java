...
import dev.c4e8.mod.modules.impl.combat.SelfTrap;
import dev.c4e8.mod.modules.impl.combat.Surround;
...
public class Step extends Module {
    ...
    public void onUpdate() {
		if (pathingPause.getValue() && BaritoneModule.isActive() || sneakingPause.getValue() && mc.player.isSneaking() || inBlockPause.getValue() && C4E8.PLAYER.insideBlock || mc.player.isInLava() || mc.player.isTouchingWater() || inWebPause.getValue() && C4E8.PLAYER.isInWeb(mc.player) || !mc.player.isOnGround() || onlyMoving.getValue() && !MovementUtil.isMoving() || surroundPause.getValue() && (Surround.INSTANCE.isOn() || SelfTrap.INSTANCE.isOn())) {
			mc.player.setStepHeight(0.6f);
			return;
		}
		...
		if (mode.getValue().equals(Mode.OldNCP) || strict) {
			...
			if (offsets != null && offsets.length > 1) {
				if (useTimer.getValue()) {
					C4E8.TIMER.set((float) getTimer(stepHeight));
					timer = true;
					packets = 2;
				}
