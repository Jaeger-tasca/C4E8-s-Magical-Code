...
import dev.c4e8.api.events.impl.*;
...
import dev.c4e8.mod.modules.impl.client.BaritoneModule;
import dev.c4e8.mod.modules.impl.client.AntiCheat;
import dev.c4e8.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), C4E8.ROTATION.rotationYaw, C4E8.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], C4E8.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - C4E8.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    private void updateNext() {
        ...
        C4E8.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        C4E8.EVENT_BUS.post(lookAtEvent);
